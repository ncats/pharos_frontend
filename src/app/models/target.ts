...
export class TargetSerializer implements PharosSerializer {
  ...
  fromJson(json: any): Target {
    ...
    if (json.nearestTclin) {
      ...
      const updateTclinDetails = (details: any) => {
        details.tClinTarget = this.fromJson(details.tClinTarget);
        details.sharedPathways = details.sharedPathways.map(p => pathwaySerializer.fromJson(p));
        details.tClinTarget.properties.push({term: details.distance, label: 'KEGG Distance'});
        details.tClinTarget.properties.push({term: details.sharedPathways.length, label: 'Common Pathways'});

        details.sharedPathways.slice(0,maxLen - 1).forEach((path, index) => {
          details.tClinTarget.properties.push({label: index + 1, term: path.name, externalLink: path.url});
        });
        if (details.sharedPathways.length === maxLen) {
          const path = details.sharedPathways[maxLen - 1];
          details.tClinTarget.properties.push({label: maxLen, term: path.name, externalLink: path.url});
        }
        if (details.sharedPathways.length > maxLen) {
          details.tClinTarget.properties.push({label: 'more', term: `${details.sharedPathways.length - maxLen + 1} more common pathways`})
        }
      };

      obj.nearestTclin.downstream.forEach(details => {
        updateTclinDetails(details);
      });
      obj.nearestTclin.upstream.forEach(details => {
        updateTclinDetails(details);
      });
    }
