  ...
  ngOnInit() {
    ...
    // todo : this component might be needed to pass the data to the child components - see setData() below
      ...
      token.components.forEach(component => {
        const dynamicComponentToken = this.componentInjectorService.getComponentToken(this.componentHost, component.token);
        const childComponent: any = this.componentInjectorService.appendComponent(this.componentHost, dynamicComponentToken);
