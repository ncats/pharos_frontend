...
/** A simple change event emitted by the RangeSliderComponent component. */
export class RangeSliderComponentChange {
  /** The RangeSliderComponent that changed. */
...
// Boilerplate for applying mixins to RangeSliderComponent.
/** @docs-private */
export class RangeSliderComponentBase {
...
export class RangeSliderComponent extends _RangeSliderComponentMixinBase
  ...
  private _increment(numSteps: number) {
    if (this.value instanceof Array) {
      this.value =
        this._clamp([
          (this.value[0] || 0) + this.step * numSteps,
          (this.value[1] || 0) + this.step * numSteps
        ], this.min, this.max);
    } else {
      this.value =
        this._clamp((this.value || 0) + this.step * numSteps, this.min, this.max);
    }
  }
  ...
  private _getSliderDimensions() {
    return this._sliderWrapper ?
      this._sliderWrapper.nativeElement.getBoundingClientRect() : null;
  }
  ...
  private calculateInitialSlideDirection(event: any) {
    if (!this._sliderDimensions) {
      return;
    }
    let offset = this.vertical ? this._sliderDimensions.top : this._sliderDimensions.left;
