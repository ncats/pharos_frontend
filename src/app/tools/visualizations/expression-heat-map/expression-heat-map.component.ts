...
export class ExpressionHeatMapComponent extends DynamicPanelComponent implements OnInit, OnChanges {
  ...
  sortedYVals: string[] = [];
  @ViewChild('tissueInput') tissueInput; // accessing the reference element

  selectedUberon: {uid: string, name: string};
  ...
  filterBySelectedTissue(tissue: string) {
    ...
    if (selectedTissue.data) {
      ...
      selectedTissue.data.ancestors.forEach(a => {
        if (this.expressionInfoService.get(a.uid)) {
          const tissueList = [];
          let count = 0;
  ...
  tissueSelected(event) {
    if (event.isUserInput) {
      this.expressionInfoService.setFocusedTissue(event.source.value, 'heatmap');
  ...
  clearFilter() {
    this.clearData();
    this.expressionInfoService.setFocusedUberon('', 'heatmap');
    this.updateChart();
  }
  ...
  updateChart(): void {
    ...
    this.heatmapData.updateDataMap(
      (f: any, args: {
        tissueSearch: boolean, filterValue: string, tissuesToShow: TissueCount[],
        ...
        if (!args.tissueSearch) {
          ...
          if (args.tissuesToShow && args.tissuesToShow.length > 0) {
            const whiteList = args.tissuesToShow.find(t => {
              return t.name === args.selectedAncestor || t.uid === args.selectedAncestor;
            }).list;
            return whiteList.includes(f.val) || f.val === args.selectedAncestor || f.val === 'Expression Type';
          } else {
            return f.data?.ancestors?.map(a => a.uid).includes(args.selectedAncestor) || f.data?.uid === args.selectedAncestor || f.val === 'Expression Type';
          }
        } else {
          return true;
        }
  ...
  changeAncestor() {
    this.expressionInfoService.setFocusedTissue(this.selectedAncestor, 'heatmap');
    this.updateChart();
  }
