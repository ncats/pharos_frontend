...
export class DonutChartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    d3.select(element)
      .call(this.donut); // draw chart in div
  }
    ...
    let chart:any = (selection: any) => {
      selection.each(function() {
        ...
        first.append('circle')
          ...
          .style('fill', colour(data[category])) // colour based on category mouse is over
...
*/
    ...
    chart.radius = function(value) {
      if (!arguments.length) return this.radius;
      this.radius = value;
      return chart;
    };
